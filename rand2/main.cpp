...
double avgLFP=-64,stim_time=0,stim_time0=0; 
int lock = 0;
int dtime = 0;
int stimType = 1; // (1 - detect DUt | 2 - detect UDt)
...
int loadTrainedWeights = 1; // 1: load trained Weights | 0: don't load trained Weights
...
//--- traing sequence 1 ---//
// sequence 1 training
int seq1_trainingStart=test2_end+1;
int seq1_trainingEnd=seq1_trainingStart+seq1_duration-1;

//--- tests after sequence 1 training ---//
// test seq1
int test3_start = seq1_trainingEnd+1;
int test3_end = test3_start+test_duration-1;

// test seq2
int test4_start = test3_end+1;
int test4_end = test4_start+test_duration-1;

//--- training sequence 2 ---//
// sequence 2 training
int seq2_trainingStart=test4_end;
int seq2_trainingEnd=seq2_trainingStart;

//--- tests after sequence 2 training ---//
// test seq1
int test5_start = seq2_trainingEnd;
int test5_end = test5_start;

// test seq2
int test6_start = test5_end;
int test6_end = test6_start;

//--- sleep ---//
int awake_end = test6_end+1;
int stage2_end=awake_end;//510000;
int stage3_end=stage2_end+sleep_duration-1;

//--- stim during sleep ---//
int up_stim_start = awake_end+1;
int up_stim_end = up_stim_start+sleep_duration-1;

//--- tests after sleep---//
// test seq1
int test7_start = stage3_end+1;
...
void print_receive_gsynapse(int m , int n, enum Cell_Type type, enum Syn_Type stype, FILE *fp){
  ...
  for(int i =0; i < cells[group_index]->num_syns; i++){
    if (cells[group_index]->syns[i]->type == stype){   
      //int from=group_cells[group_cells[group_index]->syns[i]->from_cell]->m;
      //fprintf(fp,"%d %d %lf ", m, from, (group_cells[group_index]->syns[i])->strength);
...
// read in trained weights at specified time
void load_trained_weights(int argc, char *argv[]){
  // check inputs
  if(argc<5){
    puts("Missing trained weights...");
    exit(1);
  }
  FILE *connections_file;
  if(!(connections_file=fopen(argv[4],"r"))){
    printf("%s file for connections does not exist or something\n",argv[4]);
    exit(1);
  }
  update_connections_from_file(&cells_info,cell_sizes,connections_file,Num_Types,cells);
  printf("trained weight data loaded...\n");
}
...
int main(int argc,char **argv){
  ...
  double gkl_s3            = gkl_awake_fix*s3_scale; //0.35; //correct1_func(s2_ach_fac    ,0.0);

  //double gkl_TC_awake      = 0.8; //correct1_func(awake_ach_fac ,0.0);
  ...
  double gkl_TC_s3         = gkl_TC_awake_fix*s3_scale; //1.0; //correct1_func(s2_ach_fac    ,0.0);
  ...
  double gkl_RE_s3         = gkl_RE_awake_fix*((2-s3_scale/2)-0.5); //correct1_func(s2_ach_fac,0.0);

  //double gh_awake       =1.0; //2.0;
  // double awake_AMPAd2      =0.2; //wake_ach_fac;
  double awake_AMPAd2_fix  =0.19; //wake_ach_fac;
  // double s2_AMPAd2         =awake_AMPAd2_fix*s2_scale*0.9; //0.28; //s2_ach_fac;
  double s2_AMPAd2         =awake_AMPAd2_fix*s2_scale*0.95; //0.28; //s2_ach_fac;
  double s3_AMPAd2         =awake_AMPAd2_fix*s3_scale*1.28; //0.28; //s2_ach_fac;

  double gh_TC_awake       =-8.0; //-20.0;
  double gh_TC_s2          =-2.0; //2.0;
  double gh_TC_s3          =-1.0; //2.0;

  //double awake_GABAd2      =0.22; //0.6; //awake_gaba_fac;
  double awake_GABAd2_fix  =0.22; //0.6; //awake_gaba_fac;
  double s2_GABAd2         =awake_GABAd2_fix*s2_scale; //0.30; //0.8; //s2_gaba_fac;
  double s3_GABAd2         =awake_GABAd2_fix*s3_scale; //0.30; //0.8; //s2_gaba_fac;

  // double awake_GABA_TC     =0.6; //awake_gaba_fac;
  double awake_GABA_TC_fix =0.6; //awake_gaba_fac;
  double s2_GABA_TC        =awake_GABA_TC_fix*s2_scale; //0.9; //s2_gaba_fac; 
  double s3_GABA_TC        =awake_GABA_TC_fix*s3_scale; //0.9; //s2_gaba_fac; 

  fmanipulations = fopen("manipulation.txt","w");

  double awake_AMPA_TC     =0.5; //awake_ach_fac;
  double s2_AMPA_TC        =0.5; //s2_ach_fac; 
  double s3_AMPA_TC        =0.5; //s2_ach_fac; 

  double gk_cx_slow_awake  = 1.0; //correct1_func(awake_ach_fac ,1.0);
  double gk_cx_slow_s2     = 1.0; //correct1_func(s2_ach_fac    ,1.0);
  double gk_cx_slow_s3     = 1.0; //correct1_func(s2_ach_fac    ,1.0);

  double gk_cx_spike_awake = 1.0; //correct1_func(awake_ach_fac ,1.0);
  double gk_cx_spike_s2    = 1.0; //correct1_func(s2_ach_fac    ,1.0);
  double gk_cx_spike_s3    = 1.0; //correct1_func(s2_ach_fac    ,1.0);
  ...
  while( t < tmax){
    ...
    // if(t>test2_end+1 && t<test2_end+2){
    if(t>10999 && t<11000){
      if(loadTrainedWeights==1){
        load_trained_weights(argc,argv);
      }
    }

    //printf("Running time %lf \n",t);
    if ((t<=seq1_trainingStart)){
    ...
    }else if ((t>seq1_trainingStart)&&(t<=seq1_trainingEnd)){
      ...
      fac_gkl_RE  = gkl_RE_awake_fix;
      fac_gkl_TC  = gkl_TC_awake_fix*0.5;
      fac_gkl     = gkl_awake_fix*1.2;
      fac_gh_TC   = gh_TC_awake*3;
      fac_gkca_cx = gk_cx_slow_awake;
      fac_gkm_cx  = gk_cx_slow_awake;
      fac_gkv_cx  = gk_cx_spike_awake;

    }else if ((t>seq1_trainingEnd)&&(t<=awake_end)){
