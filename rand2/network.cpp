...
// code to update AMPA_D3 wiehgts to trained weight states
void update_connections_from_file(Cell_Info *****cells_info,Pair* cell_sizes,FILE *connections_file,int Num_Types,CellSyn **cells){
  ...
  // fix to do proper check to make sure updated weight neuron dims match previous dims
  if(fscanf(connections_file,"%d\n",&Num_Types) != 1){
    printf("bad input file could not get number of types\n");
    exit(1);
  }
  ...
  while(1){
    ...
    while(1){
      ...
      if(syn_type_lit == "AMPA_D3"){
        for(int e = 0; e < cells[get_cell_index(type,x_loc,y_loc)]->num_syns; e++){
          if(cells[get_cell_index(type,x_loc,y_loc)]->syns[e]->from_cell==get_cell_index(in_type,in_x_loc,in_y_loc) && cells[get_cell_index(type,x_loc,y_loc)]->syns[e]->type==E_AMPA_D3){
            cout << "inside if2" << endl;
            if(strength == 0.000010){
              cout << "AMPA strength at min: " << x_loc << " " << in_x_loc << endl;
            }
            if(mini_s == 0.000009){
              cout << "Mini strength at min: " << x_loc << " " << in_x_loc << endl;
            }
            ((AMPA_D3*)cells[get_cell_index(type,x_loc,y_loc)]->syns[e])->g_AMPA0 = strength;
            cells[get_cell_index(type,x_loc,y_loc)]->syns[e]->strength = strength;
            cells[get_cell_index(type,x_loc,y_loc)]->syns[e]->mini_s = mini_s;
            cells[get_cell_index(type,x_loc,y_loc)]->syns[e]->mini_fre = mini_fre;
          }
        }
      }
    }
