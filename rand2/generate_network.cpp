...
#define BTYPE   3             //Boundary Condition
...
/*          int radius = CN.C[c].radius_max+1;
       ...
       for (int tx=max(sc_x-radius,0); tx<min(sc_x+radius,TX); tx++) //particular connections between neurons
            for (int ty=max(sc_y-radius,0); ty<min(sc_y+radius,TY); ty++) { 
             ...
             int maxradius = CN.C[c].radius_max;
             if (CT.cell[CN.C[c].from].name!=CT.cell[CN.C[c].to].name)
	       maxradius = CN.C[c].radius_max+1;

	     if ( d>=CN.C[c].radius_min && d<=maxradius){
...
*/
	...
	//if (CT.cell[CN.C[c].from].name=="TC")
	//	cerr<<radius_x<<endl;
          
     	//=====================add boundary conditions in the code by Yina=============================
       int radius_x = CN.C[c].radius_max;
       int radius_y = 0;

       int radius_x1 = -radius_x;
       int radius_x2 = radius_x;
       int boundarybias=0;

       if ((CT.cell[CN.C[c].from].name!=CT.cell[CN.C[c].to].name)&&(CT.cell[CN.C[c].to].name=="CX"||CT.cell[CN.C[c].to].name=="CXa"||CT.cell[CN.C[c].to].name=="CX6"))
       {	   
	 // cerr<<radius_x;
	 if(radius_x%2!=0){
	   if((TX/FX)%2!=0)
	       radius_x1 = -radius_x+1;
	    else radius_x1=-radius_x-1;
	   }
	 else{
	    if((TX/FX)%2!=0)
	       radius_x1 = -radius_x-1;
       ...
       if ((CT.cell[CN.C[c].from].name==CT.cell[CN.C[c].to].name)&&(CT.cell[CN.C[c].to].name=="CX"||CT.cell[CN.C[c].to].name=="CXa"||CT.cell[CN.C[c].to].name=="CX6"))
       {	   
	  // cerr<<CT.cell[CN.C[c].from].name;
	 //	 boundarybias=-2; //for flow(mirror) boundary connection
       }
       
      for (int ii=radius_x1; ii<=+radius_x2; ii++) //particular connections between neurons
              for (int jj=-radius_y; jj<=+radius_y; jj++) {	     
            
	     int tx = b(BTYPE+boundarybias,TX,sc_x+ii);
             int ty = b(BTYPE+boundarybias,TY,sc_y+jj);
   
	     if (tx<0||ty<0||tx>(TX-1)||ty>(TY-1)) continue;
	    // cerr<<CT.cell[CN.C[c].from].name<<","<<tx<<","<<ty<<endl;

 	    //add by Yina: for mirror connections,the neurons in the boundary should connect to the mirrored itself!!!
	     if((BTYPE+boundarybias)==1 && (sc_x+ii<0||sc_x+ii>TX-1)){
	       ;
	      }else{
	     if (sc_x==tx && sc_y==ty && CT.cell[CN.C[c].from].name==CT.cell[CN.C[c].to].name) continue; //never connect neuron to itself
	       }
...
int main(int argc, char *argv[]){
  ...
  if (argc==2) 
    ...
    load_3D_subnet(argv[2]);
    load_3D_distances(argv[1]);
