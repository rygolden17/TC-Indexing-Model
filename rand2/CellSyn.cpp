...
void CellSyn::calc(double x, double *y_ini, double *f_ini, int step){
  ...
  // testing Seq1 (ABCDE)
  if((x>=test1_start&&x<=test1_end)||(x>=test3_start&&x<test3_end)||(x>=test5_start&&x<test5_end)||(x>=test7_start&&x<test7_end)){
    if ((stime>=0)&&(stime<10)) {
  ...
  // testing Seq2 (EDCBA)
  if((x>=test2_start&&x<test2_end)||(x>=test4_start&&x<test4_end)||(x>=test6_start&&x<test6_end)||(x>=test8_start&&x<test8_end)){
    if ((stime>=0)&&(stime<10)) {
  ...
  if(x<=(up_stim_start-1) && x<=up_stim_start){
    stimCount=0;
  }

  // stim during down state
  if(stimType==2){  
  ...
  // apply index stimulation (ABCDE)
  if(x>=up_stim_start && x<up_stim_end && (stimCount%2==1)){
    if((x>=stim_time) && (x<stim_time+10)){
      if(this->cell_type == E_CX && this->m>=input_start_neuron+0+gap && this->m<=input_end_neuron+0+gap){
        current = current + input_strength;
      }
    }
    if((x>=stim_time+10) && (x<stim_time+20)){
      if(this->cell_type == E_CX && this->m>=input_start_neuron+10+gap && this->m<=input_end_neuron+10+gap){
        current = current + input_strength;
      }
    }
    if((x>=stim_time+20) && (x<stim_time+30)){
      if(this->cell_type == E_CX && this->m>=input_start_neuron+20+gap && this->m<=input_end_neuron+20+gap){
        current = current + input_strength;
      }
    }
    if((x>=stim_time+30) && (x<stim_time+40)){
      if(this->cell_type == E_CX && this->m>=input_start_neuron+30+gap && this->m<=input_end_neuron+30+gap){
        current = current + input_strength;
      }
    }
    if((x>=stim_time+40) && (x<stim_time+50)){
      if(this->cell_type == E_CX && this->m>=input_start_neuron+40+gap && this->m<=input_end_neuron+40+gap){
        current = current + input_strength;
      }
    }
